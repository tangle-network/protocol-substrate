...
fn test_basic_set_pool_weight() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(AnonymityMiningRewards::set_pool_weight(800));
		assert_eq!(AnonymityMiningRewards::get_pool_weight(), 800);

		// Set pool weight to 500
		assert_ok!(AnonymityMiningRewards::set_pool_weight(500));
		assert_eq!(AnonymityMiningRewards::get_pool_weight(), 500);
	})
}
...
fn test_basic_timestamp_change() {
	new_test_ext().execute_with(|| {
		...
		let _start_timestamp = AnonymityMiningRewards::get_current_timestamp();
		Timestamp::set_timestamp(1);
		let curr_timestamp = AnonymityMiningRewards::get_current_timestamp();
...
fn test_basic_get_virtual_reward_balance() {
	new_test_ext().execute_with(|| {
		...
		let prev_virtual_balance =
			AnonymityMiningRewards::get_virtual_balance(&AnonymityMiningRewards::account_id());
		...
		assert_ok!(Currencies::update_balance(
			RuntimeOrigin::root(),
			AnonymityMiningRewards::account_id(),
			reward_currency_id,
			new_reward_balance,
		));
...
fn test_basic_get_expected_return_varying_timestamp() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(AnonymityMiningRewards::set_pool_weight(10000));
		...
		let start_expected_return = AnonymityMiningRewards::get_expected_return(
			&AnonymityMiningRewards::account_id(),
			amount,
		);
		...
		let mid_expected_return = AnonymityMiningRewards::get_expected_return(
			&AnonymityMiningRewards::account_id(),
			amount,
		);
		...
		let end_expected_return = AnonymityMiningRewards::get_expected_return(
			&AnonymityMiningRewards::account_id(),
			amount,
		);
...
fn test_basic_get_expected_return_varying_pool_weights() {
	new_test_ext().execute_with(|| {
		...
		let init_expected_return = AnonymityMiningRewards::get_expected_return(
			&AnonymityMiningRewards::account_id(),
			amount,
		);
		...
		assert_ok!(AnonymityMiningRewards::set_pool_weight(20000));

		// Calc: 10000000(1 - e^-1/2)
		let another_expected_return = 3934693;
		let other_expected_return = AnonymityMiningRewards::get_expected_return(
			&AnonymityMiningRewards::account_id(),
			amount,
		);
		...
		assert_ok!(AnonymityMiningRewards::set_pool_weight(5000));

		// Calc: 10000000(1 - e^-2)
		let final_expected_return = 8646647;
		let last_expected_return = AnonymityMiningRewards::get_expected_return(
			&AnonymityMiningRewards::account_id(),
			amount,
		);
...
fn test_basic_swap() {
	new_test_ext().execute_with(|| {
		...
		let pallet_ap_balance_before =
			Currencies::free_balance(ap_currency_id, &AnonymityMiningRewards::account_id());
		...
		// Calc: 10000000(1 - e^-1)
		let expected_return = AnonymityMiningRewards::get_expected_return(
			&AnonymityMiningRewards::account_id(),
			amount,
		);
		...
		// conduct swap
		assert_ok!(AnonymityMiningRewards::swap(
			RuntimeOrigin::signed(sender_account_id.clone()),
			sender_account_id.clone(),
			amount
		));

		let tokens_sold = AnonymityMiningRewards::get_tokens_sold();
