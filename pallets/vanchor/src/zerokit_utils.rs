// This crate provides cross-module useful utilities (mainly type conversions) not necessarily
// specific to RLN
...
pub fn str_to_fr(input: &str, radix: u32) -> Fr {
	...
	if radix == 10 {
		BigUint::from_str_radix(&input_clean, radix).unwrap().try_into().unwrap()
	} else {
...
pub fn bytes_le_to_fr(input: &[u8]) -> (Fr, usize) {
	...
	(Fr::from(BigUint::from_bytes_le(&input[0..el_size])), el_size)
}

pub fn bytes_be_to_fr(input: &[u8]) -> (Fr, usize) {
	let el_size = fr_byte_size();
	(Fr::from(BigUint::from_bytes_be(&input[0..el_size])), el_size)
}

pub fn fr_to_bytes_le(input: &Fr) -> Vec<u8> {
	let input_biguint: BigUint = (*input).into();
	let mut res = input_biguint.to_bytes_le();
	//BigUint conversion ignores most significant zero bytes. We restore them otherwise
	// serialization will fail (length % 8 != 0)
...
pub fn fr_to_bytes_be(input: &Fr) -> Vec<u8> {
	...
	// BigUint conversion ignores most significant zero bytes. We restore them otherwise
	// serialization might fail Fr elements are stored using 64 bits nimbs
	while res.len() != fr_byte_size() {
		res.insert(0, 0);
	}
	res
