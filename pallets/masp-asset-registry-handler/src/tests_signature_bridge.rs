...
use webb_proposals::{
	substrate::{FungibleAssetAddProposal, TokenRemoveProposal, WrappingFeeUpdateProposal},
	FunctionSignature, SubstrateTargetSystem,
};

const ADD_WRAPPED_FUNGIBLE_ASSET_FUNCTION_SIG: FunctionSignature =
	FunctionSignature(0u32.to_be_bytes());
const REMOVE_WRAPPED_FUNGIBLE_ASSET_FUNCTION_SIG: FunctionSignature =
	FunctionSignature(1u32.to_be_bytes());
const ADD_WRAPPED_NFT_ASSET_FUNCTION_SIG: FunctionSignature = FunctionSignature(2u32.to_be_bytes());
const REMOVE_WRAPPED_NFT_ASSET_FUNCTION_SIG: FunctionSignature =
	FunctionSignature(3u32.to_be_bytes());
...
fn make_add_wrapped_fungible_asset_proposal(
...
#[test]
fn should_add_wrapped_fungible_asset_with_sig_succeed() {
	...
	new_test_ext_initialized(src_id, r_id, add_token_resource_bytes).execute_with(|| {
		...
		let header = make_proposal_header(r_id, ADD_WRAPPED_FUNGIBLE_ASSET_FUNCTION_SIG, nonce);
		let add_wrapped_fungible_asset_proposal_bytes =
			make_add_wrapped_fungible_asset_proposal(header, "meme".to_string(), first_token_id);
