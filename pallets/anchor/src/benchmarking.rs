...
use frame_support::{assert_ok, traits::{OnInitialize, OriginTrait, Currency, Get}};
...
// pub fn setup_environment<T:Config>(curve: Curve) -> Vec<u8> {
...
// 	<T as pallet_hasher::Pallet>::Call::<T>::force_set_parameters(RawOrigin::Root.into(), params.to_bytes());
// 	// 2. Initialize MerkleTree pallet.
// 	<<T as pallet_mt::Pallet>::OnInitialize<u64>>::on_initialize(1);
...
// 		}
// 	};
// 	<T as pallet_verifier::Pallet>::Call::<T>::force_set_parameters(RawOrigin::Root.into(), verifier_key_bytes);
...
benchmarks! {
	...
	// 	<T as pallet_balances::Pallet<_>>::Call::<T>::set_balance(RawOrigin::Root.into(), caller.clone(), 100_000_000.into(), 0.into());
	// 	let src_chain_id = 1;
	...
	// 	let deposit_size: u32 = 1_000_000;
	// 	let depth = <T as pallet_mt::Config>::MaxTreeDepth::get();
	// 	let tree_id = <T as Pallet<_>>::Call::<T>::create(RawOrigin::Root.into(), deposit_size.into(), depth, 2,  1.into()).unwrap();

	// 	<T as Pallet<_>>::deposit(
	...
	// 	let tree_root = <T as pallet_mt::Pallet<_>>::Call::<T>::get_root(tree_id).unwrap();
	...
	// )
	// verify {
	// 	assert_eq!(<<T as pallet_mixer::Config>::Currency as MultiCurrency<T::AccountId>>::total_balance(1u32.into(), &recipient_account_id), deposit_size.into())
	// }
