...
benchmarks! {
	...
	set_parameters {
		...
		Maintainer::put::<T::AccountId>(caller.clone());

		<<T as Config>::Currency as Currency<T::AccountId>>::make_free_balance_be(&caller, std::u32::MAX.into());

		Deposit::put::<Option<DepositDetails<T::AccountId, DepositBalanceOf<T>>>>(Some(DepositDetails{
			depositor,
			deposit:1_000_000.into()
		}));
	...
	verify {
		assert_last_event::<T>(Event::MaintainerSet(caller, new_maintainer).into());
	}


	force_set_parameters {
		let caller = T::ForceOrigin::successful_origin();
		let depositor: T::AccountId = account("depositor", 0, SEED);
		let parameters = vec![0u8;std::u32::MAX as usize];
...
mod tests {
	...
	fn test_benchmarks() {
		ExtBuilder::default().build().execute_with(|| {
			assert_ok!(Pallet::<Test>::test_benchmark_set_parameters());
			assert_ok!(Pallet::<Test>::test_benchmark_set_maintainer());
			assert_ok!(Pallet::<Test>::test_benchmark_force_set_parameters());
			assert_ok!(Pallet::<Test>::test_benchmark_force_set_maintainers());
		});
	}
}
