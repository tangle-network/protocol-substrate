...
fn should_fail_to_create_new_linkable_tree_if_not_root() {
	new_test_ext().execute_with(|| {
		...
		let depth = TREE_DEPTH as u8;
		assert_err!(
			LinkableTree::create(Origin::signed(account::<AccountId>("", 1, SEED)), max_edges, depth,),
			BadOrigin,
		);
	});
}
...
fn should_be_able_to_change_the_maintainer() {
	new_test_ext().execute_with(|| {
		...
		let current_maintainer_account_id = LinkableTree::maintainer();
		...
		assert_ok!(LinkableTree::force_set_maintainer(
			Origin::root(),
...
fn should_fail_to_change_the_maintainer_if_not_the_current_maintainer() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(current_maintainer_account_id, LinkableTree::maintainer());
	});
}
...
fn should_be_able_to_add_neighbors_and_check_history() {
	...
	new_test_ext().execute_with(|| {
		...
		let height = 1;
		let neighbor_index_before: u32 = LinkableTree::curr_neighbor_root_index((tree_id, src_chain_id));
		...
		let neighbor_index_after: u32 = LinkableTree::curr_neighbor_root_index((tree_id, src_chain_id));
