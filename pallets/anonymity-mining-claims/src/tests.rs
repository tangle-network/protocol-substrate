...
fn should_init_and_update_roots() {
	new_test_ext().execute_with(|| {
		...
		let max_edges = 2u8;
		let depth = 30u8;
		let _tree_id = AnonymityMiningClaims::_create(None, depth, max_edges, 0u32, 1u32).unwrap();
...
fn should_create_pallet() {
	new_test_ext().execute_with(|| {
		...
		let max_edges = 2u8;
		let depth = 30u8;
		let call = AnonymityMiningClaims::_create(None, depth, max_edges, 0u32, 1u32);
		assert_ok!(call);
	})
}
...
fn circom_should_complete_30x2_reward_claim_with_json_file() {
	new_test_ext().execute_with(|| {
		...
		let max_edges = 2u8;
		let depth = 30u8;
		let tree_id = AnonymityMiningClaims::_create(None, depth, max_edges, 0u32, 1u32).unwrap();
