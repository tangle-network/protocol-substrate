...
pub fn generate_proofs() -> (Vec<u8>, Vec<u8>, Vec<u8>, Vec<Vec<u8>>, Vec<u8>, Vec<u8>) {
	...
	let (leaf_private, leaf, nullifier_hash) = setup_leaf_circomx5(&params5, rng);
	...
	let (pk, vk) = setup_groth16_circuit_circomx5::<_, ark_bn254::Bn254, TREE_DEPTH>(&mut rng, circuit.clone());
