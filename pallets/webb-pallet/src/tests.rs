...
use webb_primitives::ValidatorSet;
use codec::Encode;
...
pub fn webb_log(log: ConsensusLog<WebbId>) -> DigestItem<H256> {
	DigestItem::Consensus(WEBB_ENGINE_ID, log.encode())
}

#[test]
fn genesis_session_initializes_authorities() {
	let want = vec![mock_webb_id(1), mock_webb_id(2), mock_webb_id(3), mock_webb_id(4)];

	new_test_ext(vec![1, 2, 3, 4]).execute_with(|| {
		let authorities = Webb::authorities();

		assert!(authorities.len() == 2);
		assert_eq!(want[0], authorities[0]);
		assert_eq!(want[1], authorities[1]);

		assert!(Webb::validator_set_id() == 0);

		let next_authorities = Webb::next_authorities();
...
fn session_change_updates_authorities() {
	new_test_ext(vec![1, 2, 3, 4]).execute_with(|| {
		...
		assert!(0 == Webb::validator_set_id());
		...
		assert!(1 == Webb::validator_set_id());

		let want = webb_log(ConsensusLog::AuthoritiesChange(ValidatorSet {
			validators: vec![mock_webb_id(3), mock_webb_id(4)],
			id: 1,
		}));
...
fn validator_set_at_genesis() {
	let want = vec![mock_webb_id(1), mock_webb_id(2)];

	new_test_ext(vec![1, 2, 3, 4]).execute_with(|| {
		let vs = Webb::validator_set();
