...
use crate as pallet_webb;

pub use webb_primitives::{crypto::AuthorityId as WebbId, ConsensusLog, WEBB_ENGINE_ID};

impl_opaque_keys! {
	pub struct MockSessionKeys {
		pub dummy: pallet_webb::Pallet<Test>,
	}
}
...
pub fn mock_webb_id(id: u8) -> WebbId {
	let buf: [u8; 33] = [id; 33];
	let pk = Public::from_raw(buf);
	WebbId::from(pk)
}

pub fn mock_authorities(vec: Vec<u8>) -> Vec<(u64, WebbId)> {
	vec.into_iter().map(|id| ((id as u64), mock_webb_id(id))).collect()
}
...
pub fn new_test_ext_raw_authorities(authorities: Vec<(u64, WebbId)>) -> TestExternalities {
