...
fn mock_anchor_creation_using_pallet_call(src_id_with_type: ChainIdWithType, resource_id: &[u8; 32]) {
	...
	Counts::<Test>::insert(src_id_with_type, 0);
...
fn make_anchor_create_proposal(
	...
	src_id_with_type: ChainIdWithType,
	resource_id: &[u8; 32],
) -> Call {
...
fn should_create_anchor_with_sig_succeed() {
	...
	new_test_ext_initialized(
		src_id_with_type,
		r_id,
		b"AnchorHandler.execute_anchor_create_proposal".to_vec(),
	)
	.execute_with(|| {
		let deposit_size = 100;
		let anchor_create_call = make_anchor_create_proposal(deposit_size, src_id_with_type, &r_id);
...
hex!("8db55b05db86c0b1786ca49f095d76344c9e6056b2f02701a7e7f3c20aabfd913ebbe148dd17c56551a52952371071a6c604b3f3abe8f2c8fa742158ea6dd7d4"
	...
	new_test_ext_initialized(
		src_id_with_type,
		r_id,
		b"AnchorHandler.execute_anchor_update_proposal".to_vec(),
	)
	.execute_with(|| {
		let prop_id = 1;
		mock_anchor_creation_using_pallet_call(src_id_with_type, &r_id);

		let root = Element::from_bytes(&[1; 32]);
		let latest_leaf_index = 5;
		let edge_metadata =
			EdgeMetadata { src_id_with_type: src_id_with_type, root, latest_leaf_index };
		assert_eq!(0, Counts::<Test>::get(src_id_with_type));
		...
		assert_eq!(1, Counts::<Test>::get(src_id_with_type));
		...
		assert_eq!(
			edge_metadata,
			<pallet_linkable_tree::EdgeList<Test>>::get(expected_tree_id, src_id_with_type)
		);

		let expected_update_record =
			UpdateRecord { tree_id: expected_tree_id, resource_id: r_id, edge_metadata };
		assert_eq!(expected_update_record, UpdateRecords::<Test>::get(src_id_with_type, 0));
	})
}
...
hex!("8db55b05db86c0b1786ca49f095d76344c9e6056b2f02701a7e7f3c20aabfd913ebbe148dd17c56551a52952371071a6c604b3f3abe8f2c8fa742158ea6dd7d4"
	...
	.execute_with(|| {
		...
		assert_eq!(2, Counts::<Test>::get(src_id_with_type));
		...
		let expected_update_record =
			UpdateRecord { tree_id: expected_tree_id, resource_id: r_id, edge_metadata };
		assert_eq!(expected_update_record, UpdateRecords::<Test>::get(src_id_with_type, 1));
	})
}
...
fn should_fail_to_whitelist_chain_already_whitelisted() {
	let chain_type = [2, 0];
	let src_id_with_type = compute_chain_id_with_type(1u32, chain_type);
	let this_chain_id = compute_chain_id_with_type(1u32, chain_type);
	let r_id = derive_resource_id(this_chain_id, b"execute_anchor_create_proposal");
	let public_uncompressed =
hex!("8db55b05db86c0b1786ca49f095d76344c9e6056b2f02701a7e7f3c20aabfd913ebbe148dd17c56551a52952371071a6c604b3f3abe8f2c8fa742158ea6dd7d4"
);
	...
	.execute_with(|| {
		assert_err!(
			SignatureBridge::whitelist_chain(Origin::root(), src_id_with_type),
	...
	.execute_with(|| {
		assert_err!(
			SignatureBridge::whitelist_chain(
				Origin::root(),
				compute_chain_id_with_type(ChainId::get(), chain_type)
			),
			pallet_signature_bridge::Error::<Test, _>::InvalidChainIdWithType
		);
	})
}
...
hex!("8db55b05db86c0b1786ca49f095d76344c9e6056b2f02701a7e7f3c20aabfd913ebbe148dd17c56551a52952371071a6c604b3f3abe8f2c8fa742158ea6dd7d4"
	...
	.execute_with(|| {
		let deposit_size = 100;
		let non_existent_r_id = derive_resource_id(this_chain_id, b"execute_anchor_crate_proposal");
		let anchor_create_call =
			make_anchor_create_proposal(deposit_size, src_id_with_type, &non_existent_r_id);
		let anchor_create_call_encoded = anchor_create_call.encode();
		let nonce = [0u8, 0u8, 0u8, 1u8];
		let prop_data =
