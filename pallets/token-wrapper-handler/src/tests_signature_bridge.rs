...
use super::{
	mock_signature_bridge::{
		assert_events, new_test_ext, Balances, Call, ChainId, Event, Origin, ProposalLifetime,
		SignatureBridge, System, Test, ENDOWED_BALANCE, RELAYER_A, RELAYER_B, RELAYER_C,
	},
	*,
};
...
fn should_update_fee_with_sig_succeed() {
	let src_id = compute_chain_id_with_type(1u32, SUBSTRATE_CHAIN_TYPE);
	let this_chain_id = compute_chain_id_with_type(5u32, SUBSTRATE_CHAIN_TYPE);
