...
//! WEBB + MMR utilties.
//!
//! While WEBB can be used completely indepentently as an additional consensus gadget,
...
//! used in conjunction with WEBB, to be able to efficiently prove any past blockchain data.
//!
//! This module contains primitives used by Polkadot implementation of the WEBB+MMR bridge,
...
pub struct MmrLeaf<BlockNumber, Hash, MerkleRoot> {
	...
	/// A merkle root of the next WEBB authority set.
	pub webb_next_authority_set: WebbNextAuthoritySet<MerkleRoot>,
...
/// Details of the next WEBB authority set.
#[derive(Debug, Default, PartialEq, Eq, Clone, Encode, Decode)]
#[cfg_attr(feature = "scale-info", derive(scale_info::TypeInfo))]
pub struct WebbNextAuthoritySet<MerkleRoot> {
	/// Id of the next set.
	///
	/// Id is required to correlate WEBB signed commitments with the validator set.
	/// Light Client can easily verify that the commitment witness it is getting is
	/// produced by the latest validator set.
	pub id: crate::ValidatorSetId,
	/// Number of validators in the set.
	///
	/// Some WEBB Light Clients may use an interactive protocol to verify only subset
	/// of signatures. We put set length here, so that these clients can verify the minimal
	/// number of required signatures.
	pub len: u32,
	/// Merkle Root Hash build from WEBB AuthorityIds.
	///
