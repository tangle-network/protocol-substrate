...
pub(crate) struct WebbKeystore(Option<SyncCryptoStorePtr>);
...
impl WebbKeystore {
	...
	/// Returns a vector of [`webb_primitives::crypto::Public`] keys which are currently supported (i.e. found
	/// in the keystore).
...
impl From<Option<SyncCryptoStorePtr>> for WebbKeystore {
	fn from(store: Option<SyncCryptoStorePtr>) -> WebbKeystore {
		WebbKeystore(store)
	}
}
...
mod tests {
	...
	use webb_primitives::{crypto, KEY_TYPE};
	use webb_test::Keyring;

	use super::WebbKeystore;
	...
	fn authority_id_works() {
		...
		let store: WebbKeystore = Some(store).into();
	...
	#[test]
	fn sign_no_keystore() {
		let store: WebbKeystore = None.into();
	...
	fn verify_works() {
		...
		assert!(WebbKeystore::verify(&alice, &sig, msg));
