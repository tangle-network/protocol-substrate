...
where
	B: Block,
{
	<<B::Header as Header>::Hashing as Hash>::hash(b"webb-dkg")
}
...
mod tests {
	...
	use super::*;
	use webb_primitives::{crypto::Signature, Commitment, MmrRootHash, VoteMessage, KEY_TYPE};
	...
	fn sign_commitment<BN: Encode, P: Encode>(who: &Keyring, commitment: &Commitment<BN, P>) -> Signature {
		...
		let webb_keystore: WebbKeystore = Some(store).into();

		webb_keystore.sign(&who.public(), &commitment.encode()).unwrap()
	}
