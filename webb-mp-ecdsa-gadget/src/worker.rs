...
use webb_primitives::{
	...
	WebbApi, Commitment, ConsensusLog, MmrRootHash, SignedCommitment, ValidatorSet, VersionedCommitment, VoteMessage,
	WEBB_ENGINE_ID, GENESIS_AUTHORITY_SET_ID,
};

use crate::{
	error,
	gossip::{topic, GossipValidator},
	keystore::WebbKeystore,
	...
	B: Block,
{
	pub client: Arc<C>,
	pub backend: Arc<BE>,
	pub key_store: WebbKeystore,
	pub signed_commitment_sender: notification::WebbSignedCommitmentSender<B>,
...
pub(crate) struct WebbWorker<B, C, BE>
where
	B: Block,
	BE: Backend<B>,
	C: Client<B, BE>,
{
	client: Arc<C>,
	backend: Arc<BE>,
	key_store: WebbKeystore,
	signed_commitment_sender: notification::WebbSignedCommitmentSender<B>,
	gossip_engine: Arc<Mutex<GossipEngine<B>>>,
	gossip_validator: Arc<GossipValidator<B>>,
	/// Min delta in block numbers between two blocks, WEBB should vote on
	min_block_delta: u32,
	...
	/// Best block a WEBB voting round has been concluded for
...
where
	...
	C::Api: WebbApi<B>,
{
	/// Return a new WEBB worker instance.
	///
	/// Note that a WEBB worker is only fully functional if a corresponding
	/// WEBB pallet has been deployed on-chain.
	///
	/// The WEBB pallet is needed in order to keep track of the WEBB authority set.
	...
	C::Api: WebbApi<B>,
{
	...
	/// a WEBB authority set change and we can't fetch the authority set from the
	/// WEBB on-chain state.
	///
	...
	fn handle_finality_notification(&mut self, notification: FinalityNotification<B>) {
		...
		if let Some(active) = self.validator_set(&notification.header) {
			...
			// the currently active WEBB voting round by starting a new one. This is
				...
				|| (active.id == GENESIS_AUTHORITY_SET_ID && self.best_webb_block.is_none())
			{
				...
				if active.id != self.last_signed_id + 1 && active.id != GENESIS_AUTHORITY_SET_ID {
					metric_inc!(self, webb_skipped_sessions);
				}
				...
				// this metric is kind of 'fake'. Best WEBB block should only be updated once we have a
				// signed commitment for the block. Remove once the above TODO is done.
				metric_set!(self, webb_best_block, *notification.header.number());
			}
		}
	...
	fn handle_vote(&mut self, round: (MmrRootHash, NumberFor<B>), vote: (Public, Signature)) {
		...
		if vote_added && self.rounds.is_done(&round) {
			if let Some(signatures) = self.rounds.drop(&round) {
				...
				self.best_webb_block = Some(round.1);

				metric_set!(self, webb_best_block, round.1);
			}
		}
	}
	...
	Id: Codec,
{
	header.digest().logs().iter().find_map(|log| {
		match log.try_to::<ConsensusLog<Id>>(OpaqueDigestItemId::Consensus(&WEBB_ENGINE_ID)) {
...
where
	B: Block,
{
	let id = OpaqueDigestItemId::Consensus(&WEBB_ENGINE_ID);
