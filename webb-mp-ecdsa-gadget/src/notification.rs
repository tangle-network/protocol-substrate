...
pub type SignedCommitment<Block> = webb_primitives::SignedCommitment<NumberFor<Block>, webb_primitives::MmrRootHash>;
...
#[derive(Clone)]
pub struct WebbSignedCommitmentSender<B>
where
	B: Block,
{
...
impl<B> WebbSignedCommitmentSender<B>
where
	B: Block,
{
...
/// The `WebbSignedCommitmentStream` entity stores the `SharedSignedCommitmentSenders` so it can be
/// used to add more subscriptions.
#[derive(Clone)]
pub struct WebbSignedCommitmentStream<B>
where
	B: Block,
{
	subscribers: SharedSignedCommitmentSenders<B>,
...
impl<B> WebbSignedCommitmentStream<B>
where
	B: Block,
{
	/// Creates a new pair of receiver and sender of signed commitment notifications.
	pub fn channel() -> (WebbSignedCommitmentSender<B>, Self) {
		let subscribers = Arc::new(Mutex::new(vec![]));
		let receiver = WebbSignedCommitmentStream::new(subscribers.clone());
		let sender = WebbSignedCommitmentSender::new(subscribers);
		(sender, receiver)
	}

	/// Create a new receiver of signed commitment notifications.
	///
	/// The `subscribers` should be shared with a corresponding `WebbSignedCommitmentSender`.
	fn new(subscribers: SharedSignedCommitmentSenders<B>) -> Self {
		Self { subscribers }
	}
